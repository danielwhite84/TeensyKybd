...
#define USBS_STR_MANUFACTURER	L"Your Name"
#define USBS_STR_PRODUCT		L"USB Serial"
...
// and group settings.
#define USBS_STR_SERIAL_NUMBER	L"_token_cfg"

// Mac OS-X and Linux automatically load the correct drivers.  On
// Windows, even though the driver is supplied by Microsoft, an
// INF file is needed to load the driver.  These numbers need to
// match the INF file.
#define USBS_VENDOR_ID		0x16C0
#define USBS_PRODUCT_ID		0x047A
...
#define USBS_TRANSMIT_FLUSH_TIMEOUT	5   /* in milliseconds */
...
#define USBS_TRANSMIT_TIMEOUT	25   /* in milliseconds */
...
// operating systems.
#define USBS_SUPPORT_ENDPOINT_HALT
...
static const uint8_t PROGMEM usbs_endpoint_config_table[] = {
	0,
...
static uint8_t PROGMEM usbs_device_descriptor[] = {
	18,					// bLength
	1,					// bDescriptorType
	0x00, 0x02,				// bcdUSB
	2,					// bDeviceClass
	0,					// bDeviceSubClass
	0,					// bDeviceProtocol
	USBS_ENDPOINT0_SIZE,				// bMaxPacketSize0
	LSB(USBS_VENDOR_ID), MSB(USBS_VENDOR_ID),		// idVendor
	LSB(USBS_PRODUCT_ID), MSB(USBS_PRODUCT_ID),	// idProduct
	0x00, 0x01,				// bcdDevice
	1,					// iManufacturer
	2,					// iProduct
	3,					// iSerialNumber
...
static uint8_t PROGMEM usbs_config1_descriptor[CONFIG1_DESC_SIZE] = {
...
struct usbs_usb_string_descriptor_struct {
	uint8_t bLength;
	uint8_t bDescriptorType;
	int16_t wString[];
};
static struct usbs_usb_string_descriptor_struct PROGMEM usbs_string0 = {
	4,
	3,
	{0x0409}
};
static struct usbs_usb_string_descriptor_struct PROGMEM usbs_string1 = {
	sizeof(USBS_STR_MANUFACTURER),
	3,
	USBS_STR_MANUFACTURER
};
static struct usbs_usb_string_descriptor_struct PROGMEM usbs_string2 = {
	sizeof(USBS_STR_PRODUCT),
	3,
	USBS_STR_PRODUCT
};
static struct usbs_usb_string_descriptor_struct PROGMEM usbs_string3 = {
	sizeof(USBS_STR_SERIAL_NUMBER),
	3,
	USBS_STR_SERIAL_NUMBER
};

// This table defines which descriptor data is sent for each specific
// request from the host (in wValue and wIndex).
static struct usbs_descriptor_list_struct {
	uint16_t	wValue;
	uint16_t	wIndex;
	const uint8_t	*addr;
	uint8_t		length;
} PROGMEM usbs_descriptor_list[] = {
	{0x0100, 0x0000, usbs_device_descriptor, sizeof(usbs_device_descriptor)},
	{0x0200, 0x0000, usbs_config1_descriptor, sizeof(usbs_config1_descriptor)},
	{0x0300, 0x0000, (const uint8_t *)&usbs_string0, 4},
	{0x0301, 0x0409, (const uint8_t *)&usbs_string1, sizeof(USBS_STR_MANUFACTURER)},
	{0x0302, 0x0409, (const uint8_t *)&usbs_string2, sizeof(USBS_STR_PRODUCT)},
	{0x0303, 0x0409, (const uint8_t *)&usbs_string3, sizeof(USBS_STR_SERIAL_NUMBER)}
};
#define USBS_NUM_DESC_LIST (sizeof(usbs_descriptor_list)/sizeof(struct usbs_descriptor_list_struct))
...
static volatile uint8_t usbs_usb_configuration=0;
...
uint8_t usbs_usb_configured(void)
{
	return usbs_usb_configuration;
}
...
int8_t usb_serial_putchar(uint8_t c)
{
	...
	if (!usbs_usb_configuration) return -1;
	// interrupts are disabled so these functions can be
	// used from the main program or interrupt context,
	// even both in the same program!
	intr_state = SREG;
	...
	timeout = UDFNUML + USBS_TRANSMIT_TIMEOUT;
	while (1) {
	...
	transmit_flush_timer = USBS_TRANSMIT_FLUSH_TIMEOUT;
	SREG = intr_state;
	return 0;
}
...
void usbs_usb_gen_vector(void)
{
	uint8_t intbits, t;
		...
		UECFG1X = EP_SIZE(USBS_ENDPOINT0_SIZE) | EP_SINGLE_BUFFER;
		UEIENX = (1<<RXSTPE);
		usbs_usb_configuration = 0;
		cdc_line_rtsdtr = 0;
        }
...
void usbs_usb_com_vector(void)
{
	...
	uint8_t	desc_length;
        ...
        if (intbits & (1<<RXSTPI)) {
			...
			list = (const uint8_t *)usbs_descriptor_list;
			for (i=0; ; i++) {
				if (i >= USBS_NUM_DESC_LIST) {
					UECONX = (1<<STALLRQ)|(1<<EPEN);  //stall
					return;
				}
				desc_val = pgm_read_word(list);
				if (desc_val != wValue) {
					list += sizeof(struct usbs_descriptor_list_struct);
					continue;
				}
				list += 2;
				desc_val = pgm_read_word(list);
				if (desc_val != wIndex) {
					list += sizeof(struct usbs_descriptor_list_struct)-2;
					continue;
				}
				list += 2;
			...
			do {
				...
				// send IN packet
				n = len < USBS_ENDPOINT0_SIZE ? len : USBS_ENDPOINT0_SIZE;
				for (i = n; i; i--) {
					UEDATX = pgm_read_byte(desc_addr++);
				}
				len -= n;
				usb_send_in();
			} while (len || n == USBS_ENDPOINT0_SIZE);
			return;
                }
		...
		if (bRequest == SET_CONFIGURATION && bmRequestType == 0) {
			usbs_usb_configuration = wValue;
			cdc_line_rtsdtr = 0;
			transmit_flush_timer = 0;
			usb_send_in();
			cfg = usbs_endpoint_config_table;
			for (i=1; i<5; i++) {
				UENUM = i;
		...
		#ifdef USBS_SUPPORT_ENDPOINT_HALT
